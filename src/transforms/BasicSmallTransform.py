...
class RandomAffineAugmentation:
    ...
    def __init__(self, angle: int = 0, shift: Tuple[int, int] = (0,0), scale: Tuple[float, float] = (1.0, 1.0), shear: Tuple[int, int] = (0,0), mean: List[float] = [0.4914, 0.4822, 0.4465], std: List[float] = [0.2023, 0.1994, 0.2010], image_size: List[int] = [32,32], padding: Union[int, Tuple[int,int], List[int]] = 0) -> None:
        """Initialize shift augmentations
        ...
        self.transform = transforms.Compose([
            transforms.Pad(padding),
            lambda x: x.convert('RGB'),
            transforms.RandomAffine(angle, shift, scale, shear),
            transforms.Resize(image_size),
            transforms.ToTensor(),
            transforms.Normalize(mean=mean, std=std)
        ])
